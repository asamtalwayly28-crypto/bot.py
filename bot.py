...
from config import DEV_ID, DEV_USERNAME, BOT_NAME
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"اهلين فيك باوامر البوت ({BOT_NAME})\n"
        f"للإستفسار - {DEV_USERNAME}\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dev_"):
        if user_id != DEV_ID:
            await query.answer("هذا الزر خاص بالمطور الأساسي فقط!", show_alert=True)
            return
        if data == "dev_stats":
            stats_text = (
                f"📊 **إحصائيات بوت {BOT_NAME}:**\n\n"
            ...
            text = "📢 **قسم الإذاعة:**\n\nأرسل الرسالة الآن ليتم نشرها لجميع المستخدمين."
    ...
    if data == "menu_m1":
        ...
        text = "🎮 كافة الألعاب الـ 37 مفعلة وتعمل بمجرد كتابة اسم اللعبة في المجموعة!"
        await query.edit_message_text(text, reply_markup=get_back_markup())
    elif data == "menu_bank":
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private":
        ...
        if user_id == DEV_ID and broadcast_mode.get(user_id, False):
            ...
            for uid in known_users:
                try:
                    ...
                    pass
            await message.reply_text(f"✅ تمت الإذاعة بنجاح إلى `{count}` وجهة.", parse_mode="Markdown")
            return

        # أوامر الأيدي والافتار في الخاص
        if text in ["ايدي", "ا", "افتاري", "اا"]:
            ...
            caption_text = (
                ...
                f"- 🪞 UsEr : {username} 🪄 .\n"
                f"- 🪞 MsG : 25 🪄 .\n"
                f"- 🪞 StA : {'owner' if user_id == DEV_ID else 'member'} 🎖️ .\n"
                f"- 🪞 ID : {user_id} 🪄 .\n"
                f"- 🪞 TITLE : لا يوجد 🪄 .\n"
                f"- 🪞 BIO : I don't chase, i attracts 🪄 ."
            )
            if photos and photos.total_count > 0:
                await message.reply_photo(photo=photos.photos[0][-1].file_id, caption=caption_text)
            else:
    ...
    if chat.type in ["group", "supergroup"]:
        ...
        if text in ["تفعيل", "تشغيل"]:
            ...
            await message.reply_text(f"✅ تم تفعيل بوت {BOT_NAME} في هذه المجموعة بنجاح!")
            return
        ...
        # 🪞 أمر الأيدي والافتار بالتنسيق المطلوب بالرد أو بدون
        if text in ["ايدي", "ا", "افتاري", "اا"]:
            target_user = message.reply_to_message.from_user if message.reply_to_message else user
            t_name = target_user.first_name if target_user else "مستخدم"
            t_username = f"@{target_user.username}" if target_user and target_user.username else "لا يوجد"
            t_id = target_user.id if target_user else 0
            ...
            caption_text = (
                ...
                f"- 🪞 UsEr : {t_username} 🪄 .\n"
                f"- 🪞 MsG : 25 🪄 .\n"
                f"- 🪞 StA : {'owner 🎖️' if t_id == DEV_ID else 'member'} 🪄 .\n"
                f"- 🪞 ID : {t_id} 🪄 .\n"
                f"- 🪞 TITLE : لا يوجد 🪄 .\n"
                f"- 🪞 BIO : I don't chase, i attracts 🪄 ."
            )
        ...
        # 🛠️ أوامر الإدارة الحقيقية (كتم، فك كتم، طرد، حظر)
        ...
        if text in GAMES_DATA:
            ...
            if f_path and os.path.exists(f_path):
                with open(f_path, 'rb') as aud:
                    await message.reply_audio(audio=aud, title=title, performer=f"البوت {BOT_NAME}")
                await wait.delete()
                os.remove(f_path)
            else:
...
def main():
    ...
    print(f"Bot {BOT_NAME} is running smoothly with clean structure...")
